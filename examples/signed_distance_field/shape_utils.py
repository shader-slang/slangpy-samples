...
def draw_circle(
    ...
    if antialiased:
        ...
        for i in range(4):
            ...
            image[:, :, i] = np.where(dist <= 1.0, color[i] * new_coverage, image[:, :, i])
    else:
...
def draw_line(
    ...
    if antialiased:
        ...
        for i in range(4):
            ...
            new_coverage = np.minimum(current + coverage, 1.0)
            image[:, :, i] = np.where(
                dist <= thickness / 2 + 1, color[i] * new_coverage, image[:, :, i]
            )
    else:
...
def draw_rotated_rect(
    ...
    for i in range(4):
        rotated[i, 0] = corners[i, 0] * cos_a - corners[i, 1] * sin_a + center_x
        rotated[i, 1] = corners[i, 0] * sin_a + corners[i, 1] * cos_a + center_y
